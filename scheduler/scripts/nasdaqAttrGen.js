...
const nasdaqAttrGen = () => {
  ...
  const nasdaqSymbolsLine1 = 'Symbol|Security Name|Market Category|Test Issue|Financial Status|Round Lot Size|ETF|NextShares';
  const otherSymbolsLine1 = 'ACT Symbol|Security Name|Exchange|CQS Symbol|ETF|Round Lot Size|Test Issue|NASDAQ Symbol';
  const optionsLine1 = 'Root Symbol|Options Closing Type|Options Type|Expiration Date|Explicit Strike Price|Underlying Symbol|Underlying Issue Name|Pending';
  ...
  for (var i=0;i<nasdaqSymbols.length;i++) {
    ...
    var symbol = line[0], name = line[1], isETF = line[6];
    attrs.push({symbol, name, isETF, optionCount: 0, optionDates: new Set(), optionPrices: new Set()});
  }
  ...
  for (var i=0;i<otherSymbols.length;i++) {
    ...
    var symbol = line[0], name = line[1], isETF = line[4];
    attrs.push({symbol, name, isETF, optionCount: 0, optionDates: new Set(), optionPrices: new Set()});
  }
  
  for (var i=0;i<options.length;i++) {
    var line = options[i].split('|');
    var symbol = line[0], date = line[3], price = line[4], undlyngSymbol = line[5]; // Symbol is formatted like AAPL1, undlyngSymbol is underlying like AAPL
